...
def gradient_descent(func_grad, init_point, step_size, num_iters):
    ...
    for i in range(num_iters):
        ...
        scaled_gradient = [step_size*v for v in curr_gradient]

        # TASK 2.1.2
        # subtract scaled gradient from current iterate
        curr_iter = map(lambda (a,b):a-b, zip(curr_iter,scaled_gradient))
...
def vector_dot(u, v):
    ...
    if len(u) != len(v):
        raise Exception('Expected u and v to be of same length.')
    else:
        return sum(map(lambda (a,b):a*b, zip(u,v)))
...
def loss_grad_calculator(X, Y, scalar_loss_deriv, beta):
    ...
    for i in range(n):
        ...
        loss_grad_on_i = [scalar_loss_deriv(inner_prod, Y[i])*X[i][j] for j in\
                           range(dim)]

        # TASK 2.3.3
        # add loss_grad_on_i (elementwise) to the sum so far
        loss_grad_sum = map(lambda (a,b):a+b, zip(loss_grad_sum,loss_grad_on_i))
...
def main():
    ...
    squared_loss = lambda z: loss_calculator(X,Y,loss['squared'],z)
    squared_loss_grad = lambda z: loss_grad_calculator\
    (X,Y,loss_deriv['squared'],z)
    squared_loss_grad_1d = lambda z,w: loss_grad_1d_calculator\
    (X,Y,loss_deriv['squared'],z,w)
    ...
    # along with the 3 functions: loss_calculator, loss_grad_calculator,
    # and loss_grad_1d_calculator
    logistic_loss = lambda z: loss_calculator(X,Y,loss['logistic'],z)
    logistic_loss_grad = lambda z: loss_grad_calculator\
    (X,Y,loss_deriv['logistic'],z)
    logistic_loss_grad_1d = lambda z,w: loss_grad_1d_calculator\
    (X,Y,loss_deriv['logistic'],z,w)
