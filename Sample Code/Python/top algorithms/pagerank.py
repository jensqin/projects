...
def main():
    ...
    adj_mat_norm[dangling == 0] = (adj_mat[dangling == 0].T/ \
                sum(adj_mat[dangling == 0].T)).T

    # TASK 1.4
    # make matrix stochastic
    rank_one_change = np.zeros((n, n))
    rank_one_change = dangling.reshape(n,1) * (np.ones(n))/n
    ...
    top_10 = np.argsort(pageranks)[-10:].tolist()
    ...
    close_to_1 = np.isclose(w, 1).nonzero()[0]
    if close_to_1.size:
    ...
    pageranks_eig = pageranks_eig*1.0/sum(pageranks_eig)
