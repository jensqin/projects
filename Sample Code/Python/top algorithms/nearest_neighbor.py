...
def get_fold_indices(n, num_folds, fold_id):
    ...
    if fold_id == num_folds-1:
        ...
        train_indices = range(0, num_folds-1*fold_size)
    else:
        # TASK 3.1.2
        # set train_indices correctly
    ...
    test_indices = range(fold_id*fold_size, (fold_id+1)*fold_size)
...
def nn_classifier(point, train_data, train_labels, k, K):
    ...
    all_three.sort(key=lambda x:x[0])
    all_three_sorted = all_three

    # TASK 3.2.2
    # after sorting extract the labels from the first k
    # triples
    nearest_k_labels = [v[2] for v in all_three_sorted[0:k]]
    ...
    for label in range(K):
        freq[label] = len([v for v in nearest_k_labels if v == label])
